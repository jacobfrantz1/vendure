...
- Stateful session identifiers are invalidated on the server after logout. The `logout` mutation deletes the current session from the DB & session cache. The `logoutEverywhere` mutation deletes all sessions for the user, enabling users to sign out of all devices.
...
- Current session deleted during `logout`, all sessions deleted during `logoutEverywhere`
